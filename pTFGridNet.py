...
class pTFGridNet(nn.Module):
    def __init__(self,
        ...
        self.first_conv_padding = nn.ConstantPad2d((0, 0, self.t_pad, 0), 0)

        self.conv = nn.Sequential(nn.Conv2d(2, emb_dim, ks, padding=(0, f_ksize // 2)),
                                  nn.BatchNorm2d(emb_dim))

        # Speaker Extractor
        self.fusion_blocks = nn.ModuleList([FusionModule(emb_dim, chunk_size, left_context, nhead=4) for _ in range(n_layers)])
    ...
    def forward(self,
        ...
        """
        # Speech Encoder
        mix = self.first_conv_padding(mix)
        ...
        # Speech Decoder
        esti = self.first_conv_padding(esti)
        esti = self.deconv(esti)[..., self.t_pad:-self.t_pad, :]  # [B, 2, T, F]
        esti = esti.permute(0, 3, 2, 1)
        return esti
...
class GridNetBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # # Sub-Band Temporal Module
        intra_rnn = self.dropout(intra_rnn)
        ...
        inter_rnn = self.inter_rnn(inter_rnn)[0] # [B * F, T, C]
        ...
        inter_rnn = self.dropout(inter_rnn)
        batch = inter_rnn
        ...
        Q = torch.cat((torch.zeros(Q.shape[0], self.left_context, Q.shape[2],
                                   device=batch.device), Q), dim=1)

        K = torch.cat((torch.zeros(K.shape[0], K.shape[1], self.left_context,
                                   device=batch.device), K), dim=2)

        V = torch.cat((torch.zeros(V.shape[0], self.left_context, V.shape[2],
                                   device=batch.device), V), dim=1)

        attn_mat = torch.matmul(Q, K) / emb_dim ** 0.5  # [B', T, T]

        mask = self.get_mask(chunk_size=self.chunk_size, mask_length=attn_mat.size()[1],
                             left_context=self.left_context).to(attn_mat.device)

        attn_mat = attn_mat.masked_fill(mask == True, -float("Inf"))
        attn_mat = nn.functional.softmax(attn_mat, dim=2)  # [B', T, T]

        V = torch.matmul(attn_mat, V)  # [B', T, C*F]
        V = V[:, self.left_context:, :]
        V = V.reshape(old_shape)  # [B', T, C, F]
        V = V.transpose(1, 2)  # [B', C, T, F]
        emb_dim = V.shape[1]
        batch = V.view([self.n_head, B, emb_dim, old_T, -1])  # [H, B, C, T, F]
...
class FusionModule(nn.Module):
    ...
    def forward(self,
        ...
        mask = self.get_mask(self.chunk_size, esti_flatten.shape[1], left_context=0).to(esti.device)
        aux_adapt = self.attn(aux, esti_flatten, esti_flatten, attn_mask=mask, need_weights=False)[0]
...
class StreamFusionModule(nn.Module):
    ...
    def forward(self,
        ...
        B_aux, C_aux, T_aux, F_aux = aux.shape
        ...
        aux = aux.reshape(B_aux, C_aux, T_aux, F_aux)
        esti = self.fusion(torch.cat((esti, aux), dim=1))  # [B, C, T, F]
        return esti
